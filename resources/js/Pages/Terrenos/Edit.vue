...
const imagePreview = ref(props.terreno.activo_fijo?.imagen ? '/storage/' + props.terreno.activo_fijo.imagen : null);

const form = useForm({
    // Activo Fijo Fields
    codigo: props.terreno.activo_fijo?.codigo || '',
    descripcion: props.terreno.activo_fijo?.descripcion || '',
    serie: props.terreno.activo_fijo?.serie || '',
    ...
    vida_util: props.terreno.activo_fijo?.vida_util || '',
    ...
    color_id: props.terreno.activo_fijo?.color_id || '',
    ...
    estado_activo_id: props.terreno.activo_fijo?.estado_activo_id || '',
    ubicacion_id: props.terreno.activo_fijo?.ubicacion_id || '',
    imagen: null,
    // Terreno Specific Fields
    area_m2: props.terreno.area_m2 || '',
    folio_real: props.terreno.folio_real || '',
    catastro: props.terreno.catastro || '',
    uso: props.terreno.uso || '',
    _method: 'PUT',
});

watch(() => form.categoria_id, async (newVal) => {
    // Only fetch new code if category changes and it's not the initial load (which we can't easily distinguish here without more state, 
    // but the user changing the dropdown is the main trigger). 
    // We should be careful not to overwrite the existing code if it belongs to the same category, 
    // but here we assume if they change category, they want the new code format.
    if (newVal && newVal !== props.terreno.activo_fijo?.categoria_id) {
         try {
...
const submit = () => {
    ...
    form.post(route('terrenos.update', props.terreno.id), {
        forceFormData: true,
        onError: (errors) => {
    ...
    <AuthenticatedLayout>
        <div class="max-w-7xl mx-auto py-6">
            <div class="flex items-center justify-between mb-8">
                <div>
                    <h1 class="text-3xl font-black text-gray-900 tracking-tight uppercase">EDITAR TERRENO</h1>
                    <p class="text-gray-500 mt-1">Modifique los datos del inmueble con Folio Real <span class="font-black text-emerald-600">{{ props.terreno.folio_real }}</span>.</p>
                </div>
                    ...
                    <div class="mt-12 flex items-center justify-between pt-10 border-t border-gray-100">
                        ...
                        <div class="flex gap-4">
                            ...
                            <PrimaryButton @click="submit" :class="{ 'opacity-25': form.processing }" :disabled="form.processing" class="!rounded-2xl px-12 !h-14 !bg-emerald-600 hover:!bg-emerald-700 shadow-xl shadow-emerald-200 font-black">
                                ...
                                <span v-else>ACTUALIZAR TERRENO</span>
                            </PrimaryButton>
                        </div>
                    </div>
                </form>
